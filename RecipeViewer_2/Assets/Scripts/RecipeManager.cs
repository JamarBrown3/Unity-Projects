...
public class RecipeManager : MonoBehaviour
{
    ...
    {
        int currentIndex = System.Array.IndexOf(recipes, recipeScript);
        int nextIndex = (currentIndex + 1) % recipes.Length; // Loop back to the first recipe
        ShowRecipe(nextIndex);
        recipeScript = recipes[nextIndex]; // Update the current recipe reference   
    }

    public void previousRecipe()
    {
        int currentIndex = System.Array.IndexOf(recipes, recipeScript);
        int previousIndex = (currentIndex - 1 + recipes.Length) % recipes.Length; // Loop back to the last recipe
        ShowRecipe(previousIndex);
        recipeScript = recipes[previousIndex]; // Update the current recipe reference
    }
}
